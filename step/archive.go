...
func runArchiveCommandWithRetry(xcodeCommandRunner xcodecommand.Runner, archiveCmd *xcodebuild.CommandBuilder, swiftPackagesPath string, logger log.Logger) (string, error) {
	output, err := runArchiveCommand(xcodeCommandRunner, archiveCmd, logger)
	if err != nil && swiftPackagesPath != "" && strings.Contains(output, cache.SwiftPackagesStateInvalid) {
		...
		return runArchiveCommand(xcodeCommandRunner, archiveCmd, logger)
	}
	return output, err
}
